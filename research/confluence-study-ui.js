document.getElementById('run').addEventListener('click',async()=>{
  ...
  try{
    const [data,prior,saved]=await Promise.all(['intraday-data.json','channel20-results.json','confluence-results.json'].map(async u=>{
    ...
    status.textContent=`16 half-tests · ${n} simulated trades. Every metric and trade matches the independent replay. Closed-candle causality checks passed.`;
