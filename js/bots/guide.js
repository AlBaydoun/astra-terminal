...
const BotGuide = {
  ...
  G: {
    ...
    live: { icon: '🛡️', art: 'shield', tagline: 'The live desk: automatic bots on the real account, on your terms — and still the hardest page to switch on.',
      tags: ['Real money', 'Bot money', 'Exit rules', 'Shape each bot', 'Four gates', 'Kill switch'],
      steps: [
        ['1️⃣', 'Connect (step 1 on the page)', 'Start START-MT5-Bridge.bat for prices and the balance. For real orders you will later start START-LIVE-TRADING.bat instead and type its six-digit code here. In shadow the ordinary bridge is enough.'],
        ['💰', 'Set the bot money', 'Type how much of the account belongs to the bots — 100 USD, or 10% — and pick the risk per trade, how many real positions at once, the daily stop and the margin cap. Everything the bots do is measured against that pool: sizes, margin, losses. Lose the pool and the desk switches itself off. The rest of the account is invisible to them.'],
        ['🎯', 'Set the exit rules', 'Stop and target of a new trade: the bot’s own levels or your % distances. Minimum target: at +0.5% (say) the desk LOCKS the stop into profit and trails it, or EXITS at the market. The picture redraws as you type. These are the defaults; any bot can get its own.'],
        ['☑', 'Tick the bots (I tick them)', 'Every bot is in the list with its record, sparkline, six readiness dots and rank. Tick any you want — a bot with few dots asks you once to confirm and is marked “little evidence”. Or press “Let the desk choose” and it takes the best-ranked bots past the door, and re-chooses every four hours.'],
        ['⇅', 'Direction', 'Buy and sell, buy only, or sell only — as the desk’s default (Exit rules card) or per bot (▸ shape it → 0 · Direction). A signal on the wrong side is dropped before it counts.'],
        ['🪆', 'Shape a bot (▸ shape it)', 'The matryoshka: open a bot → 0 direction → 1 tick its MARKETS → press “▸ pairs” on a market to go inside and tick its PAIRS → 2 tick its TIMEFRAMES → 3 give it its own stops and exit rules. Anything you leave alone stays AUTOMATIC. An untouched bot mirrors its paper twin exactly; a bot you shaped is run by the desk itself, with the bot’s own strategy, on the markets, pairs and timeframes you ticked.'],
        ['🌍', 'Where — the desk’s defaults', 'Markets, timeframes and blocked pairs for every bot you did not shape. The matrix shows every bot × market with the pair count; click a cell to switch that combination off. “What fits the money” shows which pairs the broker’s smallest order can trade at your risk.'],
        ['🛡️', 'Arm the desk', 'Tick the three acknowledgements, type ARM THE DESK. Every chosen bot goes on the real account in SHADOW: each order is worked out from the bot money and written down; nothing is sent. The “What is happening” panel appears: each bot with a pulsing ring while it scans, signals → passed your rules → passed the ceilings → written down, the bot money in motion, and the ticker of events.'],
        ['👀', 'Read the shadow log for days', 'The desk’s log says what each bot would have done and, once per bot and pair, why something was refused. Compare it with the paper trades. Change anything while it runs — the desk re-arms itself with the new choice.'],
        ['🔴', 'Go live', 'Start START-LIVE-TRADING.bat, enter its code in step 1, type TRADE REAL MONEY. From then on a desk bot may place a real order — inside the bot money, the exit rules and the hard ceilings. Real positions appear as cards and on the chart in gold, marked REAL; drag a stop or target there and confirm, the broker is told.'],
        ['🛑', 'Stopping', 'Back to shadow = no new orders, the desk keeps watching the open trades. Switch off = every desk bot disarmed. STOP EVERYTHING at the top = the kill switch. None of them closes an open position — close it from its card or the chart.'],
      ],
      settings: [
        ['Bot money', 'A number or a percentage of the balance.', 'The bots can neither size from nor lose anything beyond it.'],
        ['Risk per trade', 'Percent of the bot money a single stop may cost.', 'What the order size is worked out from.'],
        ['Positions at once / daily stop / margin cap', 'How many real trades, how much may be lost today, how much margin may be tied up.', 'Three separate brakes on the same pool.'],
        ['Stop and target of a new trade', 'The bot’s own levels, or your fixed percentages.', 'Your distances win when you want every trade to look the same.'],
        ['Minimum target · lock or exit', 'What the desk does the moment a trade is that far in profit.', 'Turns a paper edge into banked money without you watching.'],
        ['Trailing stop', 'Follows the best price at a gap of your choosing, after the lock or from the first tick.', 'Only ever tightens, never widens, never removed.'],
        ['I tick them / Let the desk choose · door', 'Who runs: your ticks, or the best-ranked bots past N of 6 readiness conditions.', 'Your ticks are your call; the automatic choice needs evidence.'],
        ['▸ shape it (per bot)', 'Markets → pairs, timeframes, own stops and exit rules for that one bot.', 'The same strategy, run by the desk on your terms.'],
        ['Hard ceilings (folded below the desk)', 'Max lots, max open, daily and total loss of the whole balance, trading hours.', 'The desk lives inside them; they cannot be crossed by anything.'],
      ],
      use: ['Press 📘 How to trade · examples at the top of the desk for worked, illustrated examples with your own numbers.', 'Every section folds (▾), moves (▲▼ or drag its title) and can take half the width (⇹); ↺ layout resets.', 'Never skip shadow. Read the desk’s log for days and compare it with the paper trades.', 'Start with bot money you could lose without a second thought; raise it only after weeks of real results.', 'A shaped bot on a timeframe it was not built for is an experiment — watch its shadow record before going live with it.', 'Real positions appear on the chart in gold, marked REAL: drag the stop or the target and confirm — the broker is told.'],
      honest: 'A backtest and a paper record do not predict future results. Real fills, spreads, gaps and slippage differ from the simulation. A stop is a request, not a promise — a gap can fill beyond it. These rules reduce risk; they do not remove it.' },
