...
const ManualAuto = {
  ...
  draft:{mode:'price',scope:'pair',direction:'both',exits:'signal',allocation:5,sl:0.5,tp:1,entries:0},
  config:null,
  view(){
    const d=this.draft;
    const select=(key,items)=>`<select data-ma="${key}">${items.map(([v,l])=>`<option value="${v}"${String(d[key])===v?' selected':''}>${l}</option>`).join('')}</select>`;
    ...
    return `<section class="manualAuto wsTradeSection"><h3>${WorkspaceUI.icon('bot')} Automatic entries · manual paper account</h3>
      <div class="botCtl"><label class="bc">Entry mode ${select('mode',[['price','At my chosen price'],['confluence','ASTRA Confluence signals']])}</label></div>
      ...
      <div data-ma-signal><fieldset data-ma-fields class="botCtl">
        ...
        <label class="bc">Stops and targets ${select('exits',[['signal','Confluence levels · 2 / 4 ATR'],['percent','My percentage distances']])}</label>
        ...
        </fieldset><div class="botCtl"><button class="bBtn go" data-ma-start>Start Confluence paper session</button><button class="bBtn danger" data-ma-stop>Stop new automatic entries</button></div>
        <p class="botNote">M15 completed-candle signals, with the existing 11:00–17:45 broker-time filter. One entry per pair per signal; later signals may add trades within your saved manual rules. The per-entry percentage is position value, not stop risk. Stop the session to change its settings or selected pair.</p>
  ...
  refresh(){
    ...
    if(shortcut)shortcut.textContent=this.running?'Manual · AUTO ON':'Manual';
    const host=document.querySelector('.manualAuto');if(!host)return;
  ...
  async start(){
    ...
    return new Promise(resolve=>{
      navigator.locks.request('astra-manual-auto-session',{ifAvailable:true},async lock=>{
        this.starting=false;
        ...
        this.status='Running · '+(c.scope==='all'?'all JustMarkets instruments':c.sym)+' · M15';
  ...
  repeat(L,s){
    const key=Feed.brokerName(s.sym)+':'+s.entryBar;
    return this.seen.has(key)||[...L.open,...L.closed].some(p=>p.meta?.manualAutoBar===s.entryBar&&Feed.brokerName(p.sym)===Feed.brokerName(s.sym));
  },
  async enter(row,version){
    ...
    return ManualOrders.exclusive(()=>{
      ...
      let why=ConfluenceBot.entryReason(s,q);
      ...
      const sig={...s,sym:row.sym,tf:'15m',manual:true,model:'Manual Auto · Confluence',meta:{...s.meta,manualAutoBar:s.entryBar},reasons:['Automatic Confluence entry',...(s.reasons||[])]};
      if(c.exits==='percent'){
      ...
      this.seen.add(Feed.brokerName(row.sym)+':'+s.entryBar);this.count++;
  ...
  async pulse(){
    ...
    try{
      const rows=await ConfluenceScanner.scan();if(!this.active(version))return;
      ...
      this.status='Running · '+scope.length+' pairs checked · '+candidates.length+' signal candidates · M15';
