...
const MarketNews = {
  ...
  init(){
    ...
    host.innerHTML=`<div class="mnShell">
      ...
      <div class="mnToolbar"><div id="mnFilters" aria-label="Filter market news">${['all','important','FX','Gold','Energy','Indices','Stocks','Crypto'].map((x,i)=>`<button data-filter="${x}" class="${i?'':'on'}" aria-pressed="${!i}">${x==='all'?'All markets':x==='important'?'⚡ High impact':x}</button>`).join('')}</div><label class="mnSearch">${this.icon('search')}<input id="mnSearch" type="search" placeholder="Search headlines…" aria-label="Search news headlines"></label></div>
